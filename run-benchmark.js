...
async function runBenchmark() {
  ...
  if (args.length < 2) {
    console.log('Usage: node run-benchmark.js <schema-path> <output-dir> [iterations] [save-results]');
    console.log('');
    ...
    console.log('  node run-benchmark.js ./prisma/schema.prisma ./generated 3 ./results.json');
    console.log('');
    console.log('Options:');
    console.log('  schema-path   Path to your Prisma schema file');
    console.log('  output-dir    Directory where generated files will be written');
  ...
  try {
    ...
    if (saveResults && fs.existsSync(saveResults)) {
      ...
      try {
        ...
        const totalTime = results.results.find(r => r.name === 'full-generation')?.duration;
        if (totalTime) {
